...
public class TakeOutProject {
        ...
        do {
            ...
            switch (mealType) {
                ...
                case "Italian":
                    ...
                    int opt = TakeOutProject.foodChoice(feedItalianFood, priceItalian);
                    TakeOutProject.finalPrice(feedItalianFood, priceItalian, opt, numberOfPeople);
                ...
                case "Chinese":
                    ...
                    opt = TakeOutProject.foodChoice(feedChineseFood, priceChinese);
                    TakeOutProject.finalPrice(feedChineseFood, priceChinese, opt, numberOfPeople);
                    isWrongMenuChoice = false;
                    break;
                ...
                case "American":
                    ...
                    opt = TakeOutProject.foodChoice(feedAmericanFood, priceAmerican);
                    TakeOutProject.finalPrice(feedAmericanFood, priceAmerican, opt, numberOfPeople);
                    isWrongMenuChoice = false;
                    break;
                default:
    ...
    public static void finalPrice(int[] feeds, double[] prices, int choice, int guestCount) {
        int trays = TakeOutProject.determineTrays(guestCount, feeds[choice-1]);
        double subTotal = TakeOutProject.getSubtotal(prices[choice - 1], trays);
        double tax = TakeOutProject.getTax(subTotal, .07);
        double tip = TakeOutProject.getTip(subTotal, .15);
        ...
        System.out.printf("Price per person: $ %.2f\n", TakeOutProject.pricePerPerson(grandTotal, guestCount));
        System.out.println("Left-over servings for the delivery person: " + TakeOutProject.determineLeftOvers(feeds[choice - 1], trays, guestCount));
    }
